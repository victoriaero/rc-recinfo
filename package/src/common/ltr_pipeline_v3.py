...
# Hybrid LTR pipeline combining lexical, dense, and optional neural reranker features.
DATA_DIR = Path('data')
...
BASE_VARIANTS = [
    # Base lexical views preserve the V2 field/operator retrieval strategy.
...
EXPANDED_VARIANTS = [
    # Expanded variants use qrel-derived terms only when expansion is available.
...
def build_train_expansions(con, queries, qrels, max_terms=4):
    # Training queries can use their own positive qrels to learn expansion terms.
    exp = {}
...
def build_global_expander(con, queries, qrels, top_terms=5):
    # Test-time expansion maps observed query terms to frequent terms from relevant entities.
...
def collect_candidates(con, qid, q, candidate_k, rrf_k, train_exp=None, global_expander=None):
    ...
    for v in VARIANTS:
        # Only variants marked as expanded receive the expanded query text.
...
def dense_features(model, q, ids, fields, batch_size):
    # Encode multiple entity representations so the model can choose the best semantic view.
...
def cross_features(model, q, ids, fields, dense_map, top_k, batch_size):
    # Cross-encoder inference is restricted to dense-selected candidates for cost control.
...
def build_matrix(con, queries, qrels, args, train_exp=None, global_expander=None, dense_model=None, cross_model=None, augment=False):
    ...
    for qid, q in tqdm(queries, desc='features', unit='query'):
        # Candidate generation stays lexical; dense/cross signals are added as features.
...
def predict(models, X):
    # Ensemble mode averages LambdaMART scores from different tree configurations.
    preds = []
    for m in models:
