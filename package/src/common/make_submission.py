...
# Baseline lexical retrieval with BM25 variants, RRF fusion, and heuristics.
DATA_DIR = Path("data")
...
VARIANTS = [
    # Field-weighted BM25 variants capture different entity-search intents.
...
def collect_candidates(con: sqlite3.Connection, query: str, candidate_k: int, rrf_k: int) -> dict[str, dict[str, float]]:
    ...
    for variant in VARIANTS:
        ...
        for entity_id, bm25_score, rank in rows:
            ...
            # SQLite BM25 returns lower scores for better matches, so store -score.
...
def heuristic_bonus(query: str, fields: tuple[str, str, str]) -> float:
    ...
    # Favor compact entities with strong title/keyword coverage.
    bonus = 0.0
