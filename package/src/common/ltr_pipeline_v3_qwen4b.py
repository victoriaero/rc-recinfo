...
# Qwen is plugged into the V3 pipeline through the existing cross-feature interface.
...
def configure_base_paths() -> None:
    # Store Qwen-trained models separately from the default V3 artifacts.
...
def load_qwen_cross_model(name: str, device: str | None = None):
    ...
    # Qwen rerankers require remote model code through SentenceTransformers.
...
def qwen_cross_features(model, q, ids, fields, dense_map, top_k, batch_size):
    # Reuse the cross-feature schema so downstream LTR code remains unchanged.
    if dense_map:
        selected = sorted(
            ids,
...
def configure_argv(argv: list[str], qwen_args: argparse.Namespace) -> list[str]:
    ...
    # Enable the full V3 hybrid stack unless a qwen-no-* flag disables it.
